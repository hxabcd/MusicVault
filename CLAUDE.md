...
adapters/processors/lyrics.py       → LRC/YRC 解析，翻译合并（inline/separate 可配），GB18030 .lrc 输出
...
| `lyrics` | `lossy_lrc_encodings`, `embed_in_metadata`, `write_lrc_file` | `["utf-8"]`, `true`, `true` |
...
- Lyrics use NetEase's `tlyric` (translation) + `ytlyric` (YRC translation). `translation_format` 控制合并方式：`"separate"`（独立行）或 `"inline"`（同行前置）。
