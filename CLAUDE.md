...
| `lyrics` | `lossy_lrc_encodings`, `embed_in_metadata`, `write_lrc_file`, `lossless_use_karaoke`, `lossy_use_karaoke`, `include_romaji`, `include_translation`, `lossless_translation_format`, `lossy_translation_format` | `["utf-8"]`, `true`, `true`, `true`, `false`, `false`, `true`, `"separate"`, `"inline"` |
...
- Lyrics 拆分为两个独立接口：`StandardLyrics`（LRC 格式，提供 `lrc`/`tlyric`/`romalrc`）和 `KaraokeLyrics`（YRC 逐字格式，提供 `yrc`/`ytlrc`/`yromalrc`）。每个都支持 `merge_translation()`、`merge_romaji()`、`merge_all()`（三行合并）。`lyrics.lossless_translation_format` / `lyrics.lossy_translation_format` 控制无损/有损翻译合并方式：`"separate"`（独立行带时间戳，lossless 默认）、`"inline"`（同行前置，lossy 默认）、`"notimestamp"`（独立行无时间戳）。`lyrics.include_translation` 控制是否合并翻译（默认开启）。罗马音启用由 `lyrics.include_romaji` 控制（默认关闭）。
