...
| `lyrics` | `lossy_lrc_encodings`, `embed_in_metadata`, `write_lrc_file`, `use_karaoke`, `include_romaji` | `["utf-8"]`, `true`, `true`, `true`, `false` |
...
- Lyrics 拆分为两个独立接口：`StandardLyrics`（LRC 格式，提供 `lrc`/`tlyric`/`romalrc`）和 `KaraokeLyrics`（YRC 逐字格式，提供 `yrc`/`ytlrc`/`yromalrc`）。每个都支持 `merge_translation()`、`merge_romaji()`、`merge_all()`（三行合并）。`translation_format` 控制翻译合并方式：`"separate"`（独立行）或 `"inline"`（同行前置）。罗马音启用由 `lyrics.include_romaji` 控制（默认关闭）。
